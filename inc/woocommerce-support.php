...
if ( ! function_exists( 'woocommerce_header_add_to_cart_fragment' ) ) {
	...
	?>
	<?php
		echo sprintf( _n('%d', '%d', $woocommerce->cart->cart_contents_count, 'converio' ), $woocommerce->cart->cart_contents_count );
	?></span>
...
// Pull down "You may be interested in…" section in checkout cart
remove_action( 'woocommerce_cart_collaterals', 'woocommerce_cross_sell_display' );
add_action( 'woocommerce_cart_collaterals', 'woocommerce_cross_sell_display', 50 );

// Remove product rating from product loop cart
